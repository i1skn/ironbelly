...
class GrinNode extends Component<Props, State> {
          ...
          onPress={() => {
            ...
            switch (state.settings.chain) {
              case MAINNET_CHAIN:
                ...
                navigation.setParams({ apiSecret: MAINNET_API_SECRET })
                break
              case FLOONET_CHAIN:
                store.dispatch({ type: 'SWITCH_TO_FLOONET' })
                navigation.setParams({ apiSecret: FLOONET_API_SECRET })
                break
            }
          }}
        >
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    if (apiSecretAfterReset && state.apiSecret != apiSecretAfterReset) {
      props.navigation.setParams({ apiSecret: '' })
      store.dispatch({ type: 'SET_API_SECRET', apiSecret: apiSecretAfterReset })
      return { apiSecret: apiSecretAfterReset }
    }
    return null
  }
  ...
  render() {
    ...
    return (
      <KeyboardAvoidingWrapper>
        ...
        <FormTextInput
          autoFocus={false}
          onChange={apiSecret => this.setState({ apiSecret })}
          onBlur={() => {
