#!/bin/bash
source ./scripts/variables_m1.sh
...
if [ -z "$platform" ] || [ "$platform" == "ios" ] || [ "$platform" == "all" ] ; then
  ...
  # Should be compiled separately until aarch64-apple-ios-sim is in Tier 3 
  # cargo build --target "$i" $rust_mode --no-default-features
  # printf "Building aarch64-apple-ios-sim target...\n";
  # cargo +nightly build -Z build-std --target aarch64-apple-ios-sim $rust_mode --no-default-features
fi
