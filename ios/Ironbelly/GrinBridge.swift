...
class GrinBridge: RCTEventEmitter {
    ...
    @objc func walletInit(_ state:String, phrase: String, password: String, isNew: Bool, resolve: RCTPromiseResolveBlock, reject: RCTPromiseRejectBlock) -> Void {

        var error: UInt8 = 0
        let cResult = c_wallet_init(state, phrase, password , isNew, &error)
        returnToReact(error:error, cResult:cResult!, resolve: resolve, reject: reject)
